...
async function montarDataAgendamento(
  ...
  let dataAgendamento = moment().format("YYYY-MM-DD");
  ...
  async function generateDateOptions() {
    ...
    while (i <= 7) {
      let numWeekDay = moment().add(i, "days").weekday();
      let dia = moment().add(i, "days").format("DD/MM/YYYY");
      let diaSemana = moment().weekday(numWeekDay).format("dddd");
      ...
      if (dataAgenda.dia === moment().format("DD/MM/YYYY")) {
    ...
    if (!dataAgendamentoDisponivel) {
      next6Days = next6Days.filter(dataAgenda => dataAgenda.dia !== todayDate);
    }
  ...
  try {
    if (
    ...
    } else if (dataAgendamento > moment().add(1, "year").format("YYYY-MM-DD")) {
      bot.sendMessage(
        msg.chat.id,
      ...
      const dataAgendamentoDisponivel = await validarDataAgendamento(parametros)
...
async function montarHorarioAgendamento(
  ...
  try {
    ...
    let message = `Aqui estão os nossos horários de agendamentos disponíveis.\n Selecione qual você deseja agendar o seu atendimento:\n\n`;
    ...
    const opcoes = horariosAgendamentos.data.datas.map((horarioAgendamento) => {
      ...
      return {
        dataAgendamento: formatDateReverse(horarioAgendamento.dataAgendamento),
        ...
        label: `${horarioAgendamento.organizacaoAgendamento.nome} - ${horarioAgendamento.profissionalAgendamento.nome} - ${dataFormatada} às ${horarioAgendamento.horarioAgendamento}`,
